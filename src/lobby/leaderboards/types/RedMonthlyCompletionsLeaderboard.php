...
class RedMonthlyCompletionsLeaderboard extends Leaderboard implements MysqlUpdate{

	public function getType() : string{
		return "red_monthly_completions";
	}
	...
	public function calculate() : void{
		Lobby::getInstance()->getSessionManager()->sendStrayRequest(new MySqlRequest("update_leaderboard_" . $this->getType(), new MySqlQuery(
			"main",
			"SELECT xuid, total_monthly FROM parkour_times WHERE course='red' ORDER BY total_monthly DESC LIMIT " . $this->getSize() . ";",
			[]
			...
			Core::getInstance()->getUserPool()->useUsers($xuids, function(array $users) use($rows) : void{
				$texts = [TextFormat::RED . TextFormat::BOLD . TextFormat::EMOJI_TROPHY . " Completions [MONTHLY] " . TextFormat::EMOJI_TROPHY . PHP_EOL . TextFormat::WHITE . "[Resets every month!]"];
				$i = 1;
				foreach($rows as $row){
