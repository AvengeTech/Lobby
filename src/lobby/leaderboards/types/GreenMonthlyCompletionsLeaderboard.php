...
class GreenMonthlyCompletionsLeaderboard extends Leaderboard implements MysqlUpdate{

	public function getType() : string{
		return "green_monthly_completions";
	}
	...
	public function calculate() : void{
		Lobby::getInstance()->getSessionManager()->sendStrayRequest(new MySqlRequest("update_leaderboard_" . $this->getType(), new MySqlQuery(
			"main",
			"SELECT xuid, total_monthly FROM parkour_times WHERE course='green' ORDER BY total_monthly DESC LIMIT " . $this->getSize() . ";",
			[]
			...
			Core::getInstance()->getUserPool()->useUsers($xuids, function(array $users) use($rows) : void{
				$texts = [TextFormat::GREEN . TextFormat::BOLD . TextFormat::EMOJI_TROPHY . " Completions [MONTHLY] " . TextFormat::EMOJI_TROPHY . PHP_EOL . TextFormat::WHITE . "[Resets every month!]"];
				$i = 1;
				foreach($rows as $row){
