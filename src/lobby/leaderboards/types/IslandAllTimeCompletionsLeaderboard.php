...
class IslandAllTimeCompletionsLeaderboard extends Leaderboard implements MysqlUpdate{

	public function getType() : string{
		return "island_alltime_completions";
	}
	...
	public function calculate() : void{
		Lobby::getInstance()->getSessionManager()->sendStrayRequest(new MySqlRequest("update_leaderboard_" . $this->getType(), new MySqlQuery(
			"main",
			"SELECT xuid, total FROM parkour_times WHERE course='island' ORDER BY total DESC LIMIT " . $this->getSize() . ";",
			[]
			...
			Core::getInstance()->getUserPool()->useUsers($xuids, function(array $users) use($rows) : void{
				$texts = [TextFormat::AQUA . TextFormat::BOLD . TextFormat::EMOJI_TROPHY . " Completions [ALL TIME] " . TextFormat::EMOJI_TROPHY];
				$i = 1;
				foreach($rows as $row){
