...
class GreenBestTimeLeaderboard extends Leaderboard implements MysqlUpdate{

	public function getType() : string{
		return "green_best_time";
	}
	...
	public function calculate() : void{
		Lobby::getInstance()->getSessionManager()->sendStrayRequest(new MySqlRequest("update_leaderboard_" . $this->getType(), new MySqlQuery(
			"main",
			"SELECT xuid, fastest FROM parkour_times WHERE course='green' ORDER BY fastest ASC LIMIT " . $this->getSize() . ";",
			[]
			...
			Core::getInstance()->getUserPool()->useUsers($xuids, function(array $users) use($rows) : void{
				$texts = [TextFormat::GREEN . TextFormat::BOLD . TextFormat::EMOJI_TROPHY . " Best Times " . TextFormat::EMOJI_TROPHY];
				$i = 1;
				foreach($rows as $row){
